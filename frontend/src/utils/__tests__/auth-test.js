...
import { getFullLoginUrl, getTokens, login, logout, refreshTokens } from "../auth";
...
describe("auth", () => {
  ...
  it("should return a valid login URL", async () => {
    ...
    expect(fullLoginUrl).toBe(
      `${loginUrl}?response_type=code&redirect_uri=${baseUrl
      }/&scope=phone+email+openid+profile&client_id=${process.env.REACT_APP_COGNITO_CLIENT_ID
